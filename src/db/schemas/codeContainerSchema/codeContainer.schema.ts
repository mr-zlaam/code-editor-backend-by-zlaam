import { boolean, text, pgTable, timestamp, serial, integer, varchar, index } from "drizzle-orm/pg-core";
import { projectSchema } from "../projectSchema";
import { workspaceSchema } from "../workspaceSchema";
export const codeContainerSchema = pgTable("codeContainers", {
  id: serial("id").primaryKey(),
  codeContainerName: varchar("codeContainerName", { length: 100 }).notNull().unique(),
  codeContainerDescription: varchar("codeContainerDescription", { length: 500 }),
  isContainerRunning: boolean("isContainerRunning").notNull().default(false),
  projectId: integer("projectId").notNull().references(() => projectSchema.id, {
    onDelete: "cascade",
  }),
