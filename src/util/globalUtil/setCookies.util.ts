...
export function setTokensAndCookies(
  user: TUSER,
  res: Response,
  ...
  if (setAccessToken) {
    accessToken = tokenGeneratorUtil.generateAccessToken(
      payLoad,
      res,
    ) as string;
    res.cookie(
      "accessToken",
      accessToken,
      appConstant.COOKIEOPTIONS.ACESSTOKENCOOKIEOPTIONS,
    );
  }
  ...
  if (setRefreshToken) {
    ...
    refreshToken = tokenGeneratorUtil.generateRefreshToken(
      payLoad,
      res,
    ) as string;
    res.cookie(
      "refreshToken",
      refreshToken,
      appConstant.COOKIEOPTIONS.REFRESHTOKENCOOKIEOPTIONS,
    );
  }
