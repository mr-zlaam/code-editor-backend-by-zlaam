...
export default {
  generateAccessToken: (
    ...
    try {
      const token = jwt.sign(
        {
          uid: payload.uid,
          OTP_TOKEN_VERSION: payload.OTP_TOKEN_VERSION,
          role: payload.role,
  ...
  generateRefreshToken: (
    ...
    try {
      const token = jwt.sign(
        { uid: payload.uid, OTP_TOKEN_VERSION: payload.OTP_TOKEN_VERSION },
        envConfig.JWT_SECRET,
        {
  ...
  generateOTPToken: (
    payload: { OTP?: string },
    res: Response,
    expiryTime?: number | ms.StringValue,
  ): string | Response => {
    try {
      const token = jwt.sign(payload, envConfig.JWT_SECRET, {
        expiresIn: expiryTime ?? appConstant.OTP_EXPIRY,
      });
      return token;
    } catch (error: unknown) {
      if (error instanceof Error)
...
export function verifyToken<T>(
  ...
  try {
    ...
    if (error instanceof Error) {
      logger.info(`ERROR::${error.message || "Invalid access Token"}`, {
        error,
      });
      ...
      return [
        Error(
          `Internal server error while verifying token :: ${error as string}`,
        ),
        null,
      ];
    }
